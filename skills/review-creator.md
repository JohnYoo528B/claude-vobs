...
记录 vault 架构变更 + 资源快照到 `_系统历史记录/`。触发词路由见 CLAUDE.md → "两个 skill 的分工" 表（权威来源）。代码级变更请用 dev-log-creator。
...
# 知识库（Obsidian Vault）
du -sh <你的 Obsidian vault 路径>/ <你的 Obsidian vault 路径>/.obsidian/ <你的 Obsidian vault 路径>/.git/ <你的 Obsidian vault 路径>/1学习笔记/ <你的 Obsidian vault 路径>/2工作文档/ <你的 Obsidian vault 路径>/_系统历史记录/ <你的 Obsidian vault 路径>/0VibeCoding开发日志/ 2>/dev/null
```
...
读取 `_系统历史记录/` 下最近一次版本文件，提取资源简报各项数值，与本次对比，填入"较上次"列。
...
```
✅ 历史记录已生成：
  → Obsidian: _系统历史记录/<版本号>版本-YYYYMMDD.md
